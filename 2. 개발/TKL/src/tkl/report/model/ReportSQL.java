...
public class ReportSQL {
	...
	static final String sql_TOTAL = "select count(*) from REPORT";
	//static final String SQL_SP = "select B.* from(select Rownum AS rn,A.* from (select *from REPORT order by R_NO desc)A)B where R_STATE=? or R_STATE=? and RN>=? and RN<=? order by R_NO desc";
